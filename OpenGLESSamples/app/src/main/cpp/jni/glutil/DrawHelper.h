 ...
 public:
  static void CheckGLError(const char *TAG) {
    ...
    if (GL_NO_ERROR != errorCode) {
      ...
      switch (errorCode) {
        case GL_INVALID_ENUM: errorInfo = "GL_INVALID_ENUM";
          break;
        case GL_INVALID_VALUE: errorInfo = "GL_INVALID_VALUE";
          break;
        case GL_INVALID_OPERATION: errorInfo = "GL_INVALID_OPERATION";
          break;
        case GL_OUT_OF_MEMORY: errorInfo = "GL_OUT_OF_MEMORY";
          break;
        case GL_INVALID_FRAMEBUFFER_OPERATION: errorInfo = "GL_INVALID_FRAMEBUFFER_OPERATION";
          break;
        default: break;
      }
  ...
  static void ConvertKitImageFmtToGL(const int &nativeFormat, GLenum &glFormat) {
    switch (nativeFormat) {
      case KIT_FMT_RGB32:glFormat = GL_RGBA;
        break;
      case KIT_FMT_RGB24:glFormat = GL_RGB;
        break;
      case KIT_FMT_NV21:glFormat = GL_LUMINANCE;
        break;
  ...
  static void UpdateTexture(const KitImage *pSrcImg,
                            const GLuint &nTextureID,
                            const int &nTextureType = -1) {
    ...
    KitImageUtils::PrintImageInfo(pSrcImg);
    if (nTextureID > 0 && KitImageUtils::IsImageValid(pSrcImg)) {
      GLenum glFormat = GL_RGBA;
      ConvertKitImageFmtToGL(pSrcImg->format, glFormat);
      glBindTexture(TEXTURE_TARGET, nTextureID);
      glTexImage2D(TEXTURE_TARGET, 0, glFormat, pSrcImg->width, pSrcImg->height,
                   0, glFormat, GL_UNSIGNED_BYTE, pSrcImg->data[0]);
      glBindTexture(TEXTURE_TARGET, GL_NONE);
    }
  }

  static void SaveRenderImage(const KitRect sRect, const GLenum format, std::string strPath) {
    ...
    switch (format) {
      case GL_RGB: myImageInfo.format = KIT_FMT_RGB24;
        break;
      case GL_RGBA: myImageInfo.format = KIT_FMT_RGB32;
        break;
      default: myImageInfo.format = KIT_FMT_RGB24;
        break;
    }
    KitImageUtils::AssignImageStrideByWidth(&myImageInfo);
    KitImageUtils::AllocImage(&myImageInfo);
    START_TIME ("SaveRenderImage glReadPixels")
      glReadPixels(0, 0, width, height, format, GL_UNSIGNED_BYTE, myImageInfo.data[0]);
  ...
  static void ViewportFitInOut(const int viewportIn[4],
                               const int texWidth,
                               int texHeight,
                               int viewportOut[4]) {
    LOGD("DrawHelper::ViewportFitInOut viewportIn (%d,%d,%d,%d) texWidth=%d texHeight=%d",
         viewportIn[0],
         viewportIn[1],
         viewportIn[2],
         viewportIn[3],
